...
int main(){
	...
	for (i = 0; i < gState.handCount[player1]; i++) {
        cardHold = gState.hand[player1][i];
        if (cardHold == copper || cardHold == silver || cardHold == gold) {
            treaCount++;
        }
    }
	temp = treaCount;
	...
	cardEffect(adventurer, firstChoice, secondChoice, thirdChoice, &gState, handpos, &bonus);
	
	printf("\nTest Player gains 2 cards: ");

	resultFromCheck = checkResult(gState.handCount[player1],tempState.handCount[player1]+2);
	if(resultFromCheck == 1)
		failTotal++;
	
	printf("\nTest Player gains 2 treasure card: ");
	resultFromCheck = checkResult(treaCount,temp+2);
	if(resultFromCheck == 1)
		failTotal++;
