...
int main(){
	...
	cardEffect(village, firstChoice, secondChoice, thirdChoice, &gState, handpos, &bonus);
	
	
	printf("\nTest player gains 1 cards: ");
	resultFromCheck = checkResult(gState.handCount[player1],tempState.handCount[player1]);
	if(resultFromCheck == 1)
		failTotal++;
	
	printf("\nTest player gains 1 cards from pile: ");
	resultFromCheck = checkResult(gState.deckCount[player1],tempState.deckCount[player1]-1);
	if(resultFromCheck == 1)
		failTotal++;
	
	printf("\nTest player has more two actions: ");
	resultFromCheck = checkResult(tempState.numActions+2,gState.numActions);
	if(resultFromCheck == 1)
		failTotal++;
	
	printf("\n---Check data of another player wasn't affected---\n");
	printf("Deck card don't change: ");
	resultFromCheck = checkResult(gState.deckCount[player2],tempState.deckCount[player2]);
	if(resultFromCheck == 1)
		failTotal++;
	
	printf("Hand card don't change: ");
	resultFromCheck = checkResult(gState.handCount[player2],tempState.handCount[player2]);
	if(resultFromCheck == 1)
		failTotal++;
